...
vless://$uuid@$domain:$xtls?flow=xtls-rprx-direct&encryption=none&security=xtls&sni=$BUG&type=tcp&headerType=none&host=$BUG#$user@vpnshopee
vless://$uuid@$domain:$xtls?flow=xtls-rprx-splice&encryption=none&security=xtls&sni=$BUG&type=tcp&headerType=none&host=$BUG#$user@vpnshopee
vless://$uuid@$domain:$xtls?encryption=none&security=xtls&sni=$BUG&type=ws&host=$BUG&path=/xrayws#$user@vpnshopee
trojan://$uuid@$domain:$xtls?sni=$BUG#$user@vpnshopee
${vmesslink1}
EOF
    cat <<EOF >>"/etc/rare/config-url/${user}"
# =======================================
# XRAY CORE CONFIG MERLIN CLASH ASUS
# By vpnshopee telegram: @vpnshopee
# =======================================
proxies:
  - {name: VLess Splice ${user}@vpnshopee, server: $BUG.$domain, port: $xtls, type: vless, flow: xtls-rprx-splice, uuid: $uuid, cipher: auto, tls: true, skip-cert-verify: true, network: tcp, sni: $BUG, udp: true}
  - {name: VLess Direct ${user}@vpnshopee, server: $BUG.$domain, port: $xtls, type: vless, flow: xtls-rprx-direct, uuid: $uuid, cipher: auto, tls: true, skip-cert-verify: true, network: tcp, sni: $BUG, udp: true}
  - {name: VLess WS ${user}@vpnshopee, server: $BUG.$domain, port: $xtls, type: vless, flow: xtls-rprx-direct, uuid: $uuid, cipher: auto, tls: true, skip-cert-verify: true, network: ws, ws-path: /xrayws, ws-headers: {Host: $BUG}, sni: $BUG, udp: true}
  - {name: Trojan ${user}@vpnshopee, server: $domain, port: $xtls, type: trojan, password: $uuid, sni: $BUG, skip-cert-verify: true, udp: true}
  - {name: VMess ${user}@vpnshopee, server: $domain, port: $xtls, type: vmess, uuid: $uuid, alterId: 0, cipher: auto, tls: true, skip-cert-verify: true, network: ws, ws-path: /xrayvws, ws-headers: {Host: $BUG}, udp: true}
  - {name: VMess SNI ${user}@vpnshopee, server: $domain, port: $xtls, type: vmess, uuid: $uuid, alterId: 0, cipher: auto, tls: true, skip-cert-verify: true, network: ws, ws-path: /xrayvws, ws-headers: {Host: $BUG}, sni: $BUG, udp: true}
port: 3333
socks-port: 23456
redir-port: 23457
allow-lan: true
...
EOF
	...
	echo -e ""
	echo -e " 🔰Pantang Larang VPNshopee"
	...
	echo -e " ❌Sharing Premium Config$NC"
    echo -e ""
	echo -e " Ip Vps        : $MYIP"
    echo -e " Domain        : $domain"
	echo -e " Bug Domain    : $BUG"    	
	echo -e ""
	echo -e "$green Link VLESS SPLICE:$NC vless://$uuid@$BUG.$domain:$xtls?flow=xtls-rprx-splice&encryption=none&security=xtls&sni=$BUG&type=tcp&headerType=none&host=$BUG#$user@vpnshopee"
    echo -e ""
    echo -e "$green Link VLESS DIRECT:$NC vless://$uuid@$BUG.$domain:$xtls?flow=xtls-rprx-direct&encryption=none&security=xtls&sni=$BUG&type=tcp&headerType=none&host=$BUG#$user@vpnshopee"
    echo -e ""
	echo -e "$green Link VLESS WS:$NC vless://$uuid@$BUG.$domain:$xtls?encryption=none&security=xtls&sni=$BUG&type=ws&host=$BUG&path=/xrayws#$user@vpnshopee"
    echo -e ""
	echo -e "$green Link TROJAN:$NC trojan://$uuid@$BUG.$domain:$xtls?sni=$BUG#$user@vpnshopee"
    echo -e ""
    echo -e "$green Link VMESS TLS:$NC ${vmesslink1}"
	echo -e ""
    echo -e "$green Link url OPENWRT/xrayN PC$NC: https://${domain}/s/${uuid}\n"
	echo -e ""
	echo -e "$green Link url ASUS Clash$NC: https://${domain}/s/${user}\n" 
	echo -e ""
	echo -e "\e[33m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
    echo ""
    read -n 1 -s -r -p "Press any key to back on menu"
...
function show-config() {
    ...
    echo -e " UUID/PASSWORD : $uuid"
	echo -e ""
    echo -e " Link url OPENWRT/xrayN PC$NC: https://${domain}/s/${uuid}"
	echo -e ""
	echo -e " Link url ASUS Clash$NC: https://${domain}/s/${user}"
    echo -e ""
    echo -e " Config :"
	echo -e " $link"  
	echo -e ""
...
echo -e ""
echo -e " [\e[31m•8\e[0m] \e[31mBACK TO MENU\033[0m"
echo -e ""
echo -e "\e[33m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
echo -e ""
